...
test('esbuildCompile ts', () => {
    ...
    const codeContent = fs
        .readFileSync(path.resolve(__dirname, './stubs/TsContent.ts'))
        .toString();
...
test('esbuildCompile tsx', () => {
    const { esbuildCompile } = scriptCompilers;
    const codeContent = fs
        .readFileSync(path.resolve(__dirname, './stubs/TsxContent.tsx'))
        .toString();
...
test('esbuildCompile jsx', () => {
    const { esbuildCompile } = scriptCompilers;
    const codeContent = fs
        .readFileSync(path.resolve(__dirname, './stubs/JsxContent.jsx'))
        .toString();
