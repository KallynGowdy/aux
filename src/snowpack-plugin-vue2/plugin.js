...
function displayError({ contents, filePath, error }) {
    const pad = (number, pad) =>
        `${Array.from(new Array(pad + 1)).join(' ')}${number}`;
    ...
    if (error.loc) {
        ...
        const lineNo = (number) =>
            ...
            pad(
                number,
                (error.loc.end.line + 1).toString().length -
        ...
        output.push(
            Array.from(
                new Array(
                    error.loc.start.column + lineNo(currentLine - 1).length
                )
            ).join(' ') + '^'
        );
        output.push(lineNo(currentLine) + allLines[currentLine]);
...
module.exports = function plugin(snowpackConfig) {
    return {
        name: '@casual-simulation/snowpack-plugin-vue2',
        resolve: {
            input: ['.vue'],
            output: ['.js', '.css'],
        },
        async load({ filePath }) {
            ...
            //     throw new Error(
            //         displayError({ error: errors[0], contents, filePath })
            //     );
            // }
            ...
            await Promise.all(
                descriptor.styles.map((stylePart) => {
                    ...
                    if (sourceMaps && css.map)
                        output['.css'].map += JSON.stringify(css.map);
                })
            );
