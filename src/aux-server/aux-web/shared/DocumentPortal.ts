...
import { DocumentPortalConfig } from './DocumentPortalConfig';
 ...
 */
export class DocumentPortal implements SubscriptionLike {
    items: DocumentNode[] = [];
    extraStyle: Object = {};

    private _dimensionTags: string[] = [];
    private _itemsUpdated = new Subject<DocumentNode[]>();
     ...
     */
    private _botItemMap: Map<string, DocumentNode> = new Map();
    private _config: DocumentPortalConfig;
    ...
    private _onSimulationAdded(sim: RemoteSimulation): void {
        ...
        if (!this._config) {
            this._config = new DocumentPortalConfig(
